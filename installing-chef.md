...
```
    cd ~/.chef/cookbooks
    chef generate cookbook recipeNameHere
```
...
```
    sudo nano /etc/hosts
```
2. From master/manager nodes run the following command to adding nodes into chef master:
```
...
```
    knife node run_list add nodeNameHere "recipe[cookbookNameHere]"
```
5. Install docker by using mydocker recipe to chef nodes (with user administrator)
```
    knife ssh "name:nodeNameHere" "sudo chef-client" -x administrator
```
...
```
    knife ssh "name:nodeNameHere" "sudo chef-client" -x administrator
    knife ssh "name:node2NameHere" "sudo chef-client" -x administrator
```
